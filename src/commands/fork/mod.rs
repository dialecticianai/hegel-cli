...
// Import from runtime submodule
use runtime::{
    check_runtime_compatibility, execute_agent, expand_tilde, AgentRuntime, RuntimeCompatibility,
};
...
pub fn handle_fork(agent_name: Option<&str>, prompt: Option<&str>, args: &[String]) -> Result<()> {
    ...
    // Execute the agent
    let output = execute_agent(&agent.name, &agent.path, &agent.runtime, &cmd_args)?;
