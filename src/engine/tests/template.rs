...
// Re-import private function made pub(crate) for testing
use crate::engine::template::validate_guide_name;
...
fn test_optional_placeholder_with_value() {
    ...
    let result = render_template("Hello {{?name}}!", &PathBuf::from("guides"), &context).unwrap();
