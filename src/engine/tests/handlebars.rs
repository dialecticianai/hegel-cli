use crate::engine::handlebars::*;
use handlebars::Handlebars;
use std::collections::HashMap;
use std::fs;
use tempfile::TempDir;

// Re-import private functions made pub(crate) for testing
use crate::engine::handlebars::{register_partials, validate_partial_name};
...
fn test_render_error_missing_partial() {
    ...
    let result = render_template_hbs("{{> nonexistent}}", guides_dir, &hbs_ctx(&HashMap::new()));
    assert!(result.is_err());
}
...
fn test_code_map_partial_precedence() {
    ...
    context.insert("code_map_style".to_string(), "monolithic".to_string());
    let result2 = render_template_hbs("{{> code_map}}", guides_dir, &hbs_ctx(&context)).unwrap();
